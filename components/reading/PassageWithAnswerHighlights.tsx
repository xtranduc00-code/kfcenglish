...
function expandToSentence(text: string, start: number, end: number): {
    start: number;
    end: number;
} {
    let s = start;
    ...
    const delimiters: Array<{
        ...
        "!\n",
        "?\n",
    ].map((d) => ({ idx: after.indexOf(d), len: d.length }));
    const paraBreak = after.indexOf("\n\n");
    if (paraBreak >= 0)
        delimiters.push({ idx: paraBreak, len: 1 });
...
function getAnswerHighlights(passage: string, correctAnswers: Record<number, string | string[]>, partStart: number, partCount: number, expandToFullSentence: boolean): Array<{
    start: number;
    end: number;
    qNums: number[];
}> {
...
export function PassageWithAnswerHighlights({ content, correctAnswers, partStart, partCount, submitted, }: {
    content: string;
    ...
    const parts = useMemo(() => {
        ...
        const ranges = getAnswerHighlights(content, correctAnswers, partStart, partCount, true);
        if (ranges.length === 0) {
            return [{ text: content, highlight: false }];
        }
        const result: Array<{
    ...
    return (<>
      {parts.map((p, i) => p.highlight ? (<span key={i} className="inline">
            <mark className="rounded bg-amber-200/90 px-0.5 text-inherit dark:bg-amber-500/40" id={"qNums" in p && p.qNums.length ? `reading-answer-${p.qNums[0]}` : undefined}>
              {p.text}
            </mark>
            {"qNums" in p && p.qNums.length > 0 && (<span className="ml-0.5 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-zinc-600 px-1 text-[10px] font-semibold text-white dark:bg-zinc-500" aria-hidden>
                {p.qNums.join(",")}
              </span>)}
          </span>) : (<span key={i}>{p.text}</span>))}
    </>);
}
