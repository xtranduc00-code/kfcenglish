...
export type Viseme = "rest" | "closed" | "open" | "wide" | "round" | "bite" | "tongue";
export type VisemeFrame = {
    atMs: number;
    viseme: Viseme;
};
const CMU_DICT: Record<string, string> = (cmuDictionary as Record<string, string>) || {};
...
function wordToPhones(word: string): string[] {
    ...
    if (direct)
        return splitPhones(direct);
    const altPrefix = `${word}(`;
    const firstAltKey = Object.keys(CMU_DICT).find((key) => key.startsWith(altPrefix));
    if (firstAltKey)
        return splitPhones(CMU_DICT[firstAltKey]);
    return guessPhonesFromGraphemes(word);
}
function phoneToViseme(phone: string): Viseme {
    return ARPABET_TO_VISEME[phone] ?? "rest";
}
export function textToVisemeFrames(text: string, opts: {
    startAtMs: number;
