...
import { textToVisemeFrames, type Viseme } from "@/features/call-ken/lib/phoneme-viseme";
...
function estimateDurationMs(text: string, speed: LipSyncSpeed): number {
    ...
    return Math.max(120, Math.min(1600, ((chars / CHARS_PER_SECOND) * 1000) / speedFactor));
}
export function usePhonemeLipSync(speed: LipSyncSpeed = "normal") {
    ...
    const enqueueText = useCallback((text: string) => {
        ...
        if (frames.length === 0)
            return;
        queueRef.current.push(...frames.map((frame) => ({ at: frame.atMs, viseme: frame.viseme })));
        queueRef.current.sort((a, b) => a.at - b.at);
        nextAtRef.current = frames[frames.length - 1].atMs;
        flushQueue();
    }, [flushQueue, speed]);
    const pushTranscriptDelta = useCallback((delta: string) => {
        if (!delta)
            return;
